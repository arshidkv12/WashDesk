<script lang="ts">
    ...
    const paymentStatusMap = $derived(_.keyBy(paymentStatusOptions, 'value'));
    const getPaymentStatusLabel = (val:any) => paymentStatusMap[val]?.label ?? 'Select Payment Status';
    ...
    {#if hasActiveFilters}
        <div class="flex flex-wrap gap-2 items-center">
            ...
            {#each Object.entries(localFilters) as [key, value]}
                {#if value && value !== ''}
                    <Badge variant="secondary" class="gap-1 pl-2 capitalize">
                        {key.replace('_', ' ')}: {String(value).replace('_', ' ')}
                        <Button
                            variant="ghost"
                            size="sm"
    ...
    {#if isAdvancedOpen}
        <div class="border rounded-lg p-4 space-y-4 animate-in fade-in duration-200">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {#if enablePaymentStatusFilter && paymentStatusOptions.length > 0}
                    <div class="space-y-2">
                        <Label for="payment_status">Payment payment Status</Label>
                        <Select.Root 
                            name="payment_status"
                            type='single'
                            value={localFilters.payment_status || ''} 
                            onValueChange={(value) => {
                                localFilters.payment_status = value; 
                                applyFilters();
                            }}
                        >
                            <Select.Trigger>
                                {localFilters.payment_status ? getPaymentStatusLabel(localFilters.payment_status) : 'Select Payment Status'}
                            </Select.Trigger>
                            <Select.Content>
                                <Select.Item value="">All Payment Statuses</Select.Item>
